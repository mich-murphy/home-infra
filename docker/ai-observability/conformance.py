...
def check_trace(trace_kind: str, experiment_name: str) -> dict[str, str]:
    ...
    root_attributes = [
        attr("session.id", f"conformance-{trace_kind}-session"),
        attr("app.agent.task.id", f"conformance-{trace_kind}-task"),
    ...
    if trace_kind == "evaluation":
        root_attributes.append(attr("app.agent.eval.identity", secrets.token_hex(32)))
    payload = {"resourceSpans": [{
        "resource": {"attributes": [
            attr("service.name", "ai-observability-conformance"),
            attr("app.agent.trace.kind", trace_kind),
        ]},
        "scopeSpans": [{"scope": {"name": "conformance", "version": "1.2.0"}, "spans": [
            {
                ...
                "attributes": root_attributes, "status": {"code": 1},
            },
            {
    ...
    if experiment is None:
        raise SystemExit(f"{experiment_name} experiment is missing")
    ...
    for _ in range(30):
        ...
        time.sleep(2)
    if len(found) != 1:
        raise SystemExit(f"expected one {trace_kind} trace, found {len(found)}")
    trace = client.get_trace(trace_id)
    roots = [span for span in trace.data.spans if span.parent_id is None]
    if len(roots) != 1 or roots[0].name != "agent.task":
        raise SystemExit(f"{trace_kind} trace does not have exactly one agent.task root")
    serialized = json.dumps(trace.to_dict())
    if canary in serialized or "privacy-canary@example.invalid" in serialized:
        raise SystemExit(f"{trace_kind} privacy transform leaked rich content or identity")
    session = json.loads(trace.info.trace_metadata.get("mlflow.trace.session", "null"))
    if session != f"conformance-{trace_kind}-session":
        raise SystemExit(f"{trace_kind} session.id was not mapped")
    return {"trace_id": trace_id, "experiment_id": experiment.experiment_id}


checks = {
    "development": check_trace("development", "agent-development"),
    "evaluation": check_trace("evaluation", "skill-evaluations"),
}
