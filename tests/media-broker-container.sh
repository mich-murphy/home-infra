#!/usr/bin/env bash
# Isolated local candidate test. It deliberately refuses every Docker context
...
observer_backend_network="media-observer-backend-${suffix}"
observer_frontend_network="media-observer-frontend-${suffix}"
secrets_volume="media-broker-secrets-${suffix}"
backend="media-broker-backend-${suffix}"
observer_backend="media-observer-backend-${suffix}"
observer="media-observer-${suffix}"
broker="media-broker-${suffix}"
image="home-infra/media-broker:test-${suffix}"
...
read -r port observer_port <<<"${ports}"
fake_backend=$(mktemp)
fake_observer_backend=$(mktemp)
cleanup() {
  docker_cmd rm -f "${broker}" "${backend}" "${observer}" "${observer_backend}" >/dev/null 2>&1 || true
  docker_cmd network rm "${network}" "${observer_backend_network}" "${observer_frontend_network}" >/dev/null 2>&1 || true
  docker_cmd volume rm "${secrets_volume}" >/dev/null 2>&1 || true
  docker_cmd image rm "${image}" >/dev/null 2>&1 || true
  rm -f "${fake_backend}" "${fake_observer_backend}"
}
trap cleanup EXIT
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/_ping":
            ...
            body = json.dumps([{
                "Id": "observer-test", "Names": ["Env-Command-Networks"],
                "Image": "image:Env-Command-Networks", "State": "running",
                "Env": ["SECRET=must-not-pass"], "Mounts": [{"Source": "/secret"}],
            }]).encode()
        else:
...
echo 'Building candidate image (failure is terminal; no fallback mode is used).'
docker_cmd build --progress plain -f "${repo_root}/services/media-broker/Dockerfile" -t "${image}" "${repo_root}/services/media-broker"
...
# Exercise the approved observer code with separate private backend and
# non-internal frontend networks, then verify Docker's host publication really
# reaches it. This uses no Docker socket or credentials.
docker_cmd network create --internal "${observer_backend_network}" >/dev/null
docker_cmd network create "${observer_frontend_network}" >/dev/null
docker_cmd run -d --name "${observer_backend}" --network "${observer_backend_network}" --network-alias docker-socket-proxy-agent-backend -v "${fake_observer_backend}:/fake.py:ro" "${base}" python3 /fake.py >/dev/null
docker_cmd run -d --name "${observer}" --network "${observer_backend_network}" -p "127.0.0.1:${observer_port}:2375" \
  --user 65532:65532 --read-only --cap-drop ALL --security-opt no-new-privileges:true \
  -v "${repo_root}/docker/init/agent-observer/observer.py:/observer.py:ro" \
  "${base}" python3 /observer.py >/dev/null
docker_cmd network connect "${observer_frontend_network}" "${observer}"
observer_ready=false
for _ in $(seq 1 30); do
  if [[ "$(curl -sS -o /dev/null -w '%{http_code}' "http://127.0.0.1:${observer_port}/_ping" || true)" == 200 ]]; then observer_ready=true; break; fi
  sleep 1
done
[[ "${observer_ready}" == true ]] || { echo 'observer did not publish a reachable ping' >&2; exit 1; }
python3 - "${observer_port}" <<'PY'
import json
import sys
import urllib.error
import urllib.request
...
status, body = get("/containers/json?all=1")
assert status == 200
value = json.loads(body)
assert value[0]["Names"] == ["Env-Command-Networks"]
assert "Env" not in value[0] and "Mounts" not in value[0]
for path in ("/containers/name/logs", "/containers/name/archive", "/events"):
    try: get(path)
    except urllib.error.HTTPError as error: assert error.code == 404
    else: raise AssertionError(path)
PY

echo 'Isolated candidate build, authenticated MCP discovery/read, unauthenticated rejection, and observer publication passed.'
